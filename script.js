...
function bearingTo(lat1, lon1, lat2, lon2) {
  ...
  // Handle longitude wrap-around across ±180°
  const adjustedΔλ =
    ((Δλ + Math.PI) % (2 * Math.PI) + 2 * Math.PI) % (2 * Math.PI) - Math.PI;

  const y = Math.sin(adjustedΔλ) * Math.cos(φ2);
  const x =
    Math.cos(φ1) * Math.sin(φ2) -
    Math.sin(φ1) * Math.cos(φ2) * Math.cos(adjustedΔλ);
