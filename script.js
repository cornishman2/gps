...
function surveyAction(e){
  ...
  if(a==='delete'){
    if(!confirm('Delete this survey and all its targets?'))return;
...
lightboxPrev.onclick=()=>{ currentLightboxIndex=(currentLightboxIndex-1+currentLightboxImages.length)%currentLightboxImages.length; showLightboxImage(); };
lightboxNext.onclick=()=>{ currentLightboxIndex=(currentLightboxIndex+1)%currentLightboxImages.length; showLightboxImage(); };
lightboxChange.onclick=()=>{
  ...
  input.onchange=(e)=>{
    ...
    lightbox.classList.remove('active'); showToast('📸 Processing new image...');
    const reader=new FileReader();
    reader.onload=(ev)=>{ currentLightboxTarget.images[currentLightboxIndex]=ev.target.result; save(); renderTargets(); showToast('✅ Image changed successfully'); };
    reader.onerror=()=>{ showToast('❌ Failed to process image'); };
    reader.readAsDataURL(file);
  };
  input.click();
...
lightboxDelete.onclick=()=>{
  ...
  currentLightboxTarget.images.splice(currentLightboxIndex,1); save(); lightbox.classList.remove('active'); renderTargets(); showToast('🗑️ Image deleted');
};
...
btnAddTarget.onclick=()=>{
  ...
  modalConfirm.onclick=()=>{
    ...
    const t={ id:uid('t_'), lat:lastPosition.coords.latitude, lng:lastPosition.coords.longitude, notes:name, description:desc, detectorist:data.detectoristName||'', detector:data.detectorUsed||'', createdAt:Date.now(), found:false, images:[] };
    open.targets.push(t); save(); renderTargets(); modal.classList.remove('active'); showToast('✅ Target added');
  };
  modalCancel.onclick=()=>{ modal.classList.remove('active'); };
};
...
btnBatch.onclick=()=>{
  ...
  if(!open){alert('⚠️ No open survey');return;}
  if(!batchInterval){
    btnBatch.innerHTML='⏹️ Stop Batch'; btnBatch.classList.remove('btn-secondary'); btnBatch.classList.add('btn-danger');
    batchInterval=setInterval(()=>{
      if(lastPosition){
        const t={ id:uid('t_'), lat:lastPosition.coords.latitude, lng:lastPosition.coords.longitude, notes:'', description:'', detectorist:data.detectoristName||'', detector:data.detectorUsed||'', createdAt:Date.now(), found:false, images:[] };
        open.targets.push(t); save(); renderTargets();
      }
    },3000);
    showToast('🔄 Batch mode started');
  }else{
    clearInterval(batchInterval); batchInterval=null; btnBatch.innerHTML='🔄 Batch Add'; btnBatch.classList.remove('btn-danger'); btnBatch.classList.add('btn-secondary'); showToast('⏹️ Batch mode stopped');
  }
};

btnFirstTarget.onclick=()=>{
  const open=getOpenSurvey(); if(!open){alert('⚠️ No open survey');return;} if(!open.targets.length){alert('⚠️ No targets');return;}
  selectedTargetId=open.targets[0].id; updateNavImmediate(); showToast('⏮️ First target');
};
btnPrevTarget.onclick=()=>{
  const open=getOpenSurvey(); if(!open){alert('⚠️ No open survey');return;} if(!open.targets.length){alert('⚠️ No targets');return;}
  if(!selectedTargetId){ selectedTargetId=open.targets[0].id; }else{ const idx=open.targets.findIndex(x=>x.id===selectedTargetId); selectedTargetId = idx>0 ? open.targets[idx-1].id : open.targets[open.targets.length-1].id; }
  updateNavImmediate(); const t=open.targets.find(x=>x.id===selectedTargetId); showToast('◀️ '+(t.notes||'Target'));
};
btnNextTarget.onclick=()=>{
  const open=getOpenSurvey(); if(!open){alert('⚠️ No open survey');return;} if(!open.targets.length){alert('⚠️ No targets');return;}
  if(!selectedTargetId){ selectedTargetId=open.targets[0].id; }else{ const idx=open.targets.findIndex(x=>x.id===selectedTargetId); selectedTargetId = idx<open.targets.length-1 ? open.targets[idx+1].id : open.targets[0].id; }
  updateNavImmediate(); const t=open.targets.find(x=>x.id===selectedTargetId); showToast('▶️ '+(t.notes||'Target'));
};
...
btnMarkFound.onclick=()=>{
  ...
  modalBody.innerHTML=`
    ...
    <div style="margin-bottom:16px">
      <label style="display:block;margin-bottom:8px;color:var(--muted);font-size:14px">Description</label>
      <textarea id="foundDesc" placeholder="Details..." style="width:100%">${escapeHtml(t.description||'')}</textarea>
    </div>
  `;
  modal.classList.add('active');
  modalConfirm.onclick=()=>{
    t.found=true; t.foundNote=document.getElementById('foundWhat').value; t.description=document.getElementById('foundDesc').value;
    save(); renderTargets(); modal.classList.remove('active'); showToast('✅ Marked as found!');
    if(confirm('📸 Add a photo?')){ addImageToTarget(t); }
  };
  modalCancel.onclick=()=>{ modal.classList.remove('active'); };
};

btnNewSurvey.onclick=()=>{
  const name=prompt('Survey name:','Field '+new Date().toLocaleDateString()); if(!name)return; createSurvey(name); showToast('✨ Survey created');
};
...
btnCloseSurvey.onclick=()=>{
  const o=getOpenSurvey(); if(!o){alert('⚠️ No open survey');return;} if(!confirm('Close the current survey?'))return;
...
btnExport.onclick=()=>{
  ...
  const a=document.createElement('a'); a.href=url; a.download=`metal_finder_${new Date().toISOString().slice(0,10)}.json`; document.body.appendChild(a); a.click();
  setTimeout(()=>{URL.revokeObjectURL(url);a.remove()},1500); showToast('💾 Data exported');
};
...
importFileEl.onchange=async e=>{
  ...
  try{
    ...
    if(obj&&obj.surveys){ if(!confirm('Import data? This will replace existing data.'))return; data=obj; save(); renderSurveys(); showToast('✅ Data imported'); }
    else alert('Invalid data format');
  }catch(err){ alert('Import failed: '+err.message); }
};
btnClear.onclick=()=>{
  if(!confirm('⚠️ Clear ALL data?'))return; if(!confirm('Are you absolutely sure?'))return;
...
function startGPS(){
  if(!navigator.geolocation){
    document.getElementById('gpsText').textContent='Not supported'; return;
  }
...
function handleOrientation(e){
  let head=null;
  if(typeof e.webkitCompassHeading==='number') head=e.webkitCompassHeading; // iOS true heading
  else if(typeof e.alpha==='number') head=(360-e.alpha); // Android approx
...
if(window.DeviceOrientationEvent){
  ...
  // Some browsers fire 'deviceorientationabsolute'; add if available
...
function updateNavImmediate(){
  ...
  // Relative bearing from phone facing (arrow up) to target
  const rel=((brg-smoothedHeading)+360)%360;
  arrowEl.style.transform=`rotate(${rel}deg)`;

  let direction='';
  const off = ((rel+540)%360)-180; // signed offset for text
  if(off>=-10&&off<=10)direction='Straight ahead';
  else if(off>10&&off<=45)direction='Slight right';
  else if(off>45&&off<=90)direction='Right';
  else if(off>90&&off<=135)direction='Sharp right';
  else if(off>135||off<-135)direction='Behind you';
  else if(off<-90&&off>=-135)direction='Sharp left';
  else if(off<-45&&off>=-90)direction='Left';
  else if(off<-10&&off>=-45)direction='Slight left';
  bearingTextEl.textContent=`${direction} • ${Math.round(d)}m`;

  if(currentScreen==='compass'&&navigator.vibrate&&d<4&&!hasVibrated){
    navigator.vibrate([200,100,200]); hasVibrated=true;
    setTimeout(()=>{ if(d>=5)hasVibrated=false; },5000);
  }
